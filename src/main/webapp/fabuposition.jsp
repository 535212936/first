...
<body>
<div id="tb">
	<!--  <button type="button" class="btn btn-warning glyphicon glyphicon-plus" onclick="addposition()">新增</button> -->
	 <button type="button" class="btn btn-danger glyphicon glyphicon-remove" onclick="deleteposition()">删除职位</button>
	 <button type="button" class="btn btn-info glyphicon glyphicon-edit" onclick="updateposition()">修改职位</button>

	  
	</div>

<input  type="hidden" id="companyid" name="companyid" value="${companyid}"  />
<table class="table" id="publishposition_info" border="1"></table>
...
function updateposition(){
	...
	var options=$("#publishposition_info").bootstrapTable("getSelections");
	if(options!=null && options.length==1){
		...
		BootstrapDialog.show({
	        title : '修改用户 ',
	        message: $('<div></div>').load("<%=request.getContextPath()%>/updatePosition.jsp?jid="+jid),
	        buttons : [ {// 设置关闭按钮
	            label : '关闭',
	            ...
	            action : function(dialogItself) {
	            	 $.ajax({
	            		url:"<%=request.getContextPath()%>/jobController/updatePosition.do",
	            		type:"post",
	            		data:$("#update_form_position").serialize(),
		        ...
		        callback : function(result) {
		            ...
		            if (result) {
		              ...
		              //  ids = ids.substring(1);
		                ...
		                $.ajax({
		                	url:"<%=request.getContextPath()%>/jobController/deletePosition.do",
		                	type:"post",
		                	data:{"ids":ids},
		                	success:function(){
		                		$("#publishposition_info").bootstrapTable("refresh");
 ...
 */
  ...
  function addposition(){
	 BootstrapDialog.show({
	        title : '发布简历 ',
	        message: $('<div></div>').load('addPosition.jsp'),
	        buttons : [ {// 设置关闭按钮
	            label : '关闭',
	            action : function(dialogItself) {
	                dialogItself.close();
...
/* 暂停发布 */
function turnStatusUp(jid){
		 $.ajax({
			url:"<%=request.getContextPath()%>/jobController/updateJobUp.do",
			type:"post",
			data:{"jid":jid},
			success:function(){
	     $("#publishposition_info").bootstrapTable("refresh");
...
/* 发布职位 */
function turnStatusDown(jid){
	
	$.ajax({
		url:"<%=request.getContextPath()%>/jobController/updateJobDown.do",
		type:"post",
		data:{"jid":jid},
		success:function(){
    $("#publishposition_info").bootstrapTable("refresh");
...
var compidyk=$("#companyid").val();
$("#publishposition_info").bootstrapTable({
	 
	 url:"<%=request.getContextPath()%>/jobController/queryPosition.do?companyid="+compidyk,
	 striped:true,//开启条纹样式
	 bordered:true,
	 search:true,//搜索框
		 ...
		 columns: [
        	...
        	formatter:function(value,row,index){
	        	...
	        	formatter:function(value,row,index){
	            	  ...
	            	  if(row.jstatus==1){
	            		  return "<button type='button' class='btn btn-warning glyphicon glyphicon-plane' onclick=turnStatusUp('"+row.jid+"')>暂停职位</button>";
	            	  }else{
	            		  return "<button type='button' class='btn btn-success glyphicon glyphicon-plane' onclick=turnStatusDown('"+row.jid+"')>发布职位</button>";
	            	  }
